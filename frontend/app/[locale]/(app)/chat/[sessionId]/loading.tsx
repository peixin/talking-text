export default function Loading() {
  return (
    <div className="flex min-h-0 flex-1 overflow-hidden">
      {/* Sidebar skeleton — hidden on mobile to match SessionSidebarClient */}
      <aside className="border-border hidden w-56 shrink-0 flex-col border-r md:flex">
        <div className="px-3 py-2">
          <div className="bg-muted h-8 animate-pulse rounded-md" />
        </div>
        <nav className="flex-1 space-y-0.5 overflow-hidden px-2 py-1">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="flex flex-col gap-1 rounded-md px-2 py-2">
              <div className="bg-muted h-3.5 w-3/4 animate-pulse rounded" />
              <div className="bg-muted/60 h-2.5 w-1/2 animate-pulse rounded" />
            </div>
          ))}
        </nav>
      </aside>

      {/* Main area skeleton */}
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        {/* Title bar */}
        <div className="border-border flex shrink-0 items-center border-b px-4 py-2.5">
          <div className="bg-muted h-4 w-32 animate-pulse rounded" />
        </div>

        {/* Messages */}
        <div className="flex flex-1 flex-col gap-3 px-4 py-4">
          <div className="bg-muted ml-auto h-10 w-2/3 animate-pulse rounded-2xl" />
          <div className="bg-muted h-12 w-3/4 animate-pulse rounded-2xl" />
          <div className="bg-muted ml-auto h-8 w-1/2 animate-pulse rounded-2xl" />
          <div className="bg-muted h-14 w-4/5 animate-pulse rounded-2xl" />
        </div>

        {/* Record button */}
        <div className="border-border flex shrink-0 flex-col items-center gap-2 border-t px-4 py-4">
          <div className="bg-muted h-16 w-16 animate-pulse rounded-full" />
          <div className="bg-muted/60 h-3 w-24 animate-pulse rounded" />
        </div>
      </div>
    </div>
  );
}
